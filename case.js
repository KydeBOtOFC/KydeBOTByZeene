...
module.exports = alpha = async (alpha, m, chatUpdate, store, antilink, antiwame, antilink2, antiwame2, set_welcome_db, set_left_db, set_open, set_close, _welcome, _left) => {
    try {
        ...
        const tanggal = moment().tz("Asia/Medan").format("dddd, ll")
        const jam = moment(Date.now()).tz('Asia/Medan').locale('id').format('HH:mm:ss z')
        const salam = moment(Date.now()).tz("Asia/Medan").locale('id').format('a')
        const isCreator = ["6287865346175@s.whatsapp.net", botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
